...
void Time_Display(uint32_t TimeVar,struct rtc_time *tm)
{
	  ...
	  /* 输出时间戳，公历时间 */
	  console_write(" UNIX时间戳 = %d 当前时间为: %d年(%s年) %d月 %d日 (星期%s)  %0.2d:%0.2d:%0.2d\r",TimeVar,
		...
		//时间戳
		sconsole_write((char *)str," UNIX TimeStamp = %d ",TimeVar);

		ILI9341_DispStringLine_EN(LINE(3),(char*)str);
		
		//日期
		sconsole_write((char *)str," Date: %d-%d-%d       ",
																						tm->tm_year, 
																						tm->tm_mon,
																						tm->tm_mday);
		...
		//生肖
		sconsole_write((char *)str," Chinese %s year      ",en_zodiac_sign[(tm->tm_year-3)%12]);
		
		ILI9341_DispStringLine_EN(LINE(6),(char*)str);
		
		//星期
		sconsole_write((char *)str," %s                  ",en_WEEK_STR[tm->tm_wday]);
		
		ILI9341_DispStringLine_EN(LINE(7),(char*)str);
		
		//时间
		sconsole_write((char *)str," Time:  %0.2d:%0.2d:%0.2d",
																						tm->tm_hour, 									
																						tm->tm_min, 
																						tm->tm_sec);
